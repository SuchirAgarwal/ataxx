...
class BoardWidget extends Pad  {
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        g.drawLine(_dim - 1, 0, _dim - 1, _dim);
        g.drawLine(0, _dim - 1, _dim, _dim - 1);

        for (char r = '1'; r <= '7'; r++) {
            for (char c = 'a'; c <= 'g'; c++) {
                int x = (c - 'a') * SQDIM + SQDIM / 2 - PIECE_RADIUS;
                int y = _dim - ((r - '1') * SQDIM + SQDIM / 2 + PIECE_RADIUS);
                if (_selectedCol == c && _selectedRow == r) {
                    g.setColor(SELECTED_COLOR);
                    int s = (c - 'a') * SQDIM;
                    int t = _dim - ((r - '1') * SQDIM + SQDIM);
                    g.fillRect(s, t, SQDIM, SQDIM);
                }
                if (_model.get(c, r) == RED) {
                    ...
                    drawBlock(g, x + PIECE_RADIUS, y + PIECE_RADIUS);
                }
            }
        }
    ...
    void drawBlock(Graphics2D g, int cx, int cy) {
        ...
        g.drawLine(cx - SQDIM / 2 + 5, cy - SQDIM / 2 + 5,
                cx + SQDIM / 2 - 5, cy + SQDIM / 2 - 5);
        g.drawLine(cx + SQDIM / 2 - 5, cy - SQDIM / 2 + 5,
                cx - SQDIM / 2 + 5, cy + SQDIM / 2 - 5);
        ...
        if (where.getButton() == MouseEvent.BUTTON1) {
            ...
            if (mouseCol >= 'a' && mouseCol <= 'g'
                ...
                if (_blockMode) {
                    ...
                    if (_selectedCol != 0) {
                        try {
                            Move m = Move.move("" + _selectedCol
                                    + _selectedRow + "-" + mouseCol + mouseRow);
                            if (_model.legalMove(m)) {
                                _commandQueue.offer(m.toString());
                            }
                            _selectedCol = _selectedRow = 0;
                        } catch (Exception e) {
